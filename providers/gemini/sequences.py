...
class GeminiSequences(ProviderAdapter):
    ...
    Each method implements the ordered steps a human would take on the Gemini page.
    Uses GeminiDOM for element location and self._e._page for Playwright execution.
    """
    ...
    async def send_prompt(self, text):
        ...
        for attempt, wait_sec in enumerate(retry_waits):
            if wait_sec > 0:
                logger.debug(f"Prompt input not found. Retrying in {wait_sec}s (attempt {attempt + 1}/{len(retry_waits)})...")
                await asyncio.sleep(wait_sec)
            for sel in prompt_selectors:
                try:
                    elem = self._e._page.locator(sel).first
    ...
    async def submit_response(self, text=None, expect_attachments=False):
        ...
        logger.debug(f"Success monitor: last seen image src = {self._e._last_seen_src[:60] if self._e._last_seen_src else 'None'}")

        if text:
            await self.send_prompt(text)
            # Submit only if we injected text
            await self._e._page.keyboard.press("Enter")
            logger.debug("Prompt submitted via Enter. Verifying submission...")
            ...
            if still_has_text:
                logger.debug("Text still in input after Enter — falling back to button click.")
                try:
                    # New UI: button[aria-label="Send message"] inside gem-icon-button.submit
                    btn = self._e._page.locator(
                    ...
                    if await btn.is_visible(timeout=2000):
                        await btn.click()
                        logger.debug("Fallback button click submitted.")
                    else:
                        logger.debug("Fallback button not visible — relying on Enter.")
                except Exception as _e:
                    logger.debug(f"Fallback click failed: {_e}")

            # CRITICAL: Dismiss "Creating content from images/files" popup
            await self.dismiss_agreement_popups()

            logger.debug("Monitoring for response...")
        else:
            logger.debug("Monitoring existing response (no prompt injected)...")
        ...
        try:
            ...
            logger.debug(f"Failed to load keyword files: {e}")
        ...
        for _ in range(90):  # 180 seconds max
            if self._e._stop_automation_event.is_set():
                logger.debug("Stop signal received during monitoring. Bailing out.")
            ...
            if status == "generating" and not resp_text:
                try:
                    locator = self._e._page.locator('section.processing-state_container--processing').first
            ...
            if resp_text and resp_text != last_logged_text and len(resp_text) > 2:
                ...
                logger.debug(f"Gemini: \"{flat[:200]}\"")
            ...
            if status == "success":
                ...
                if has_started_generating:
                    ...
                    logger.debug("Gemini page was unexpectedly reset during generation.")
                ...
                if not text:
                    logger.debug("Gemini reset detected during Redo monitoring. Triggering recovery...")
                ...
                if not input_empty:
                    if not hasattr(self, '_reset_watchdog_start') or self._reset_watchdog_start is None:
                        ...
                        logger.debug("Prompt still remains in input box after 6s. Submission likely failed.")
                ...
                if expect_attachments and attachment_count == 0:
                    logger.debug("Expected attachments disappeared. Env reset detected.")
                    return {"status": "reset", "message": "Attachments missing during monitoring."}

                logger.debug("Waiting for conversation to appear...")
            else:
    ...
    async def is_generating(self) -> bool:
        ...
        if not self._e.is_running or not self._e._page:
            return False
    ...
    async def get_last_response(self) -> dict:
        ...
        try:
            text = await self._e._page.evaluate("""
                () => {
            ...
            """)
            still_generating = await self._e._page.is_visible('button[aria-label="Stop response"]')
    ...
    async def redo_response(self):
        ...
        btn_info = await self._e._page.evaluate('''() => {
        ...
        try_again_info = await self._e._page.evaluate('''() => {
        ...
        logger.debug(f"Redo triggered: {result}")
    ...
    async def stop_response(self):
        ...
        logger.debug("Attempting to stop response via 'stop' icon...")

        stopped = await self._e._page.evaluate('''() => {
        ...
        if stopped:
            logger.debug("Stop command sent successfully.")
            return {"status": "success", "message": "Response stop command triggered."}
        else:
            logger.debug("Stop icon not found.")
    ...
    async def new_chat(self, target_url: str = None):
        ...
        if current_target and "gemini.google.com/gem/" in current_target:
            logger.debug(f"Gem URL detected: {current_target}")
        ...
        if result == "NOT_FOUND":
            logger.debug("New Chat button not found — navigating to /app as fallback.")
            await self._e.navigate("https://gemini.google.com/app")
            await asyncio.sleep(2.0)
            return {"status": "success", "message": "Navigated to /app as fallback."}

        logger.debug(f"New Chat clicked ({result}). Waiting for URL to return to /app...")
        ...
        url_before = self._e._page.url
        already_clean = url_before.rstrip("/").endswith("/app")
        if not already_clean:
            deadline = time.time() + 8.0
            while time.time() < deadline:
                await asyncio.sleep(0.3)
                current_url = self._e._page.url
                if current_url.rstrip("/").endswith("/app"):
                    logger.debug(f"new_chat: URL returned to /app ({current_url})")
                    break
            else:
                logger.debug(f"new_chat: URL did not return to /app within 8s (still {self._e._page.url}) — proceeding anyway.")
        else:
            logger.debug("new_chat: already at /app, no wait needed.")
        ...
        try:
            ...
            for selector in popup_selectors:
                ...
                if await btn.is_visible(timeout=1500):
                    logger.debug(f"Popup detected. Clicking: {selector}")
                    await btn.click()
    ...
    async def apply_settings(self, model_name=None, tool_name=None, thinking_level=None):
        ...
        if not self._caps:
            try:
                ...
                if res.get("status") == "success":
                    ...
                    logger.debug("apply_settings: lazy _caps populated via discover_capabilities")
            except Exception as exc:
                logger.debug(f"apply_settings: lazy discover failed (non-fatal): {exc}")
        ...
        try:
            ...
            if model_name:
                ...
                await self._e._page.click('button[data-test-id="bard-mode-menu-button"]')
                await asyncio.sleep(0.8)
                clicked = await self._e._page.evaluate('''(name) => {
            ...
            if thinking_level:
                logger.debug(f"Applying thinking level: {thinking_level}")
                await self._e._page.click('button[data-test-id="bard-mode-menu-button"]')
                await asyncio.sleep(0.8)

                # Trigger sub-menu via JS mouseenter (Playwright hover() collapses the menu)
                expanded = await self._e._page.evaluate('''() => {
                ...
                if expanded:
                    await asyncio.sleep(0.8)
                    clicked = await self._e._page.evaluate('''(level) => {
            ...
            # 3. Apply Tool
            if tool_name:
                logger.debug(f"Applying tool: {tool_name}")
                # Open Tools drawer
                opened_drawer = False
                for sel in ['button[aria-label="Upload & tools"]', 'button.toolbox-drawer-button']:
                    try:
                        ...
                        if await loc.is_visible(timeout=1500):
                            await loc.click()
                            logger.debug(f"Toolbox drawer opened via: {sel}")
                            opened_drawer = True
                            break
                    except Exception:
                        pass
                if not opened_drawer:
                    logger.debug("Toolbox drawer button not found — trying JS fallback")
                    await self._e._page.evaluate('''() => {
                ...
                try:
                    ...
                    await asyncio.sleep(0.5)
                    drawer_opened = True
                    logger.debug(f"Tool drawer open (upload buttons visible), looking for: {tool_name}")
                except Exception:
                    logger.debug("Upload buttons not visible after drawer open attempt")
                ...
                clicked = await self._e._page.evaluate(_js_match_upload(tool_name), tool_name)

                if not clicked:
                    logger.debug(f"Pass 1 (upload base) miss for '{tool_name}' — trying more-upload expansion")
                    # Pass 2: expand "More uploads" then retry (Photos, Notebooks)
                    more_up = self._e._page.locator('button.more-upload-button').first
                    if await more_up.is_visible(timeout=2000):
                        await more_up.click()
                        logger.debug("More uploads expanded, waiting for sub-items...")
                        await asyncio.sleep(1.5)
                        clicked = await self._e._page.evaluate(_js_match_upload(tool_name), tool_name)
                        if not clicked:
                            await self._e._page.keyboard.press("Escape")
                ...
                if not clicked:
                    ...
                    clicked = await self._e._page.evaluate(_js_match_ai(tool_name), tool_name)

                if not clicked:
                    logger.debug(f"Pass 3 (ai base) miss for '{tool_name}' — trying more-tools expansion")
                    # Pass 4: expand "More tools" then retry (Create music, Guided learning)
                    more_tools = self._e._page.locator('button.more-tools-button').first
            ...
            logger.debug(f"Apply settings failed: {e}")
            return {"status": "error", "message": str(e)}
    ...
    async def discover_capabilities(self):
        ...
        try:
            ...
            current_model_el = await self._e._page.query_selector(
            ...
            try:
                ...
                logger.debug("Model menu did not appear.")

            await asyncio.sleep(0.5)

            # Extract model names — text lives in span.label inside gem-menu-item-content
            results["models"] = await self._e._page.evaluate('''() => {
                return Array.from(
            ...
            results["current_thinking_level"] = await self._e._page.evaluate('''() => {
                const item = document.querySelector('gem-menu-item[value="thinking_level"]');
                if (!item) return null;
            ...
            if expanded:
                ...
                results["thinking_levels"] = await self._e._page.evaluate('''() => {
                    const parent = document.querySelector('gem-menu-item[value="thinking_level"]');
                    if (!parent) return [];
                    let el = parent.nextElementSibling;
                    while (el) {
            ...
            logger.debug("Attempting to open Tools drawer...")
            opened = False
            for locator in [
                self._e._page.locator('button[aria-label="Upload & tools"]').first,
                self._e._page.locator('button.toolbox-drawer-button').first,
            ]:
                try:
            ...
            try:
                await self._e._page.wait_for_selector('toolbox-drawer-item', timeout=5000)
            except Exception:
                logger.debug("Tools drawer did not appear.")
            await asyncio.sleep(0.5)
            ...
            upload_base: list = await self._e._page.evaluate(f'() => ({_UPLOAD_LABEL_JS})')
            ...
            has_more_uploads = await self._e._page.evaluate('''() => {
            ...
            if has_more_uploads:
                ...
                try:
                    await self._e._page.click('button.more-upload-button')
                    await asyncio.sleep(2.0)
                    all_upload: list = await self._e._page.evaluate(f'() => ({_UPLOAD_LABEL_JS})')
                    base_set = set(upload_base)
                    sub_tools["More uploads"] = [t for t in all_upload if t not in base_set]
                except Exception as exc:
                    logger.debug(f"More-uploads sub-scan failed: {exc}")

            # ── Step C: base AI tools (Create image, Canvas) — before expanding ──
            ai_base: list = await self._e._page.evaluate('''() => {
            ...
            has_more_tools = await self._e._page.evaluate('''() => {
            ...
            if has_more_tools:
                ...
                try:
                    await self._e._page.click('button.more-tools-button')
                    await asyncio.sleep(0.5)
                    ai_all: list = await self._e._page.evaluate('''() => {
                        return Array.from(document.querySelectorAll('toolbox-drawer-item'))
                            .map(i => {
                                const el = i.querySelector('.label.gem-menu-item-label')
                                        || i.querySelector('.label.gds-label-l')
                    ...
                    logger.debug(f"More-tools sub-scan failed: {exc}")
    ...
    async def download_images(self, save_dir, naming_cfg, extra_meta=None):
        ...
        last_resp_handle = await self._e._page.evaluate_handle('''() => {
            const allResps = Array.from(document.querySelectorAll('model-response, structured-content-container.model-response-text, message-content'));
            const responses = allResps.filter(el => !allResps.some(parent => parent !== el && parent.contains(el)));
        ...
        for retry in range(20):  # 20 * 0.5s = 10s max
            ...
            for img in imgs:
                ...
                if src and (natural_w > 250 or (w > 250 and natural_w > 50)):
                    ...
                    logger.debug(f"IMG-DETECTION: src={src[:60]}, w={w:.1f}, natural_w={natural_w}, dup={is_dup}")
        ...
        logger.debug(f"Downloading {len(valid_imgs)} images...")
        ...
        def get_image_ahash(path):
            try:
                ...
                logger.debug(f"Failed to calculate aHash: {e}")
                return None
        ...
        for idx in range(len(valid_imgs)):
            try:
                ...
                if not img_ready or img is None:
                    ...
                    logger.debug(f"DL-DIAG: Image not ready after 10s (h={h_val}, w={w_val}, complete={comp_val}, naturalW={nat_val}). Skipping.")
                    continue
                ...
                logger.debug(f"DL-DIAG: viewport={viewport_info}")
                ...
                dialog_diag = await self._e._page.evaluate('''(imgEl) => {
                ...
                logger.debug(f"DL-DIAG: dialog={dialog_diag}")
                ...
                if not dl_btn_found:
                    ...
                    for _wait in range(12):  # 12 * 0.5s = 6s max
                        dl_btn_found = await self._e._page.evaluate('''(imgEl) => {
                            const dialog = document.querySelector('mat-dialog-container');
                            const overlay = document.querySelector('.cdk-overlay-container .cdk-overlay-pane');
                            const searchScope = dialog || overlay || (imgEl ? imgEl.closest('single-image') || imgEl.closest('.image-container') || imgEl.parentElement : document);
                        ...
                        if dl_btn_found:
                            logger.debug(f"DL-DIAG: button appeared after {(_wait+1)*0.5:.1f}s polling")
                            break
                        await asyncio.sleep(0.5)

                # ── STEP 5: Execute download (button path or blob fallback) ──
                if not dl_btn_found:
                    logger.debug("DL-DIAG: No download button found after polling. Using canvas extraction.")
                    ...
                    for _hires_wait in range(20):  # 20 * 0.5s = 10s max
                        hires_nw = await self._e._page.evaluate('''() => {
                        ...
                        if hires_nw > 512:
                            logger.debug(f"DL-DIAG: Dialog hi-res image ready (naturalWidth={hires_nw}) after {_hires_wait * 0.5:.1f}s.")
                            break
                        logger.debug(f"DL-DIAG: Waiting for dialog hi-res image... (naturalWidth={hires_nw}, attempt {_hires_wait+1}/20)")
                        await asyncio.sleep(0.5)
                    else:
                        logger.debug("DL-DIAG: Dialog hi-res image did not load after 10s. Aborting canvas extraction.")
                        await self._e._page.keyboard.press("Escape")
                        await asyncio.sleep(0.5)
                        continue

                    # Extract image pixels directly via canvas while dialog is still open
                    img_bytes = await self._e._page.evaluate('''async (imgEl) => {
                        try {
                            const dialogImg = document.querySelector('img[data-test-id="trusted-image"]') ||
                                              document.querySelector('mat-dialog-container img.generated-image') ||
                                              document.querySelector('mat-dialog-container img') ||
                    ...
                    if img_bytes:
                        ...
                        if file_sz < 1024 * 1024:
                            ...
                            try:
                                ...
                                logger.debug(f"Failed to delete small file: {rm_err}")
                            continue
                            ...
                            for old_ahash in seen_hashes:
                                ...
                                if distance <= 3:
                                    ...
                                    try:
                                        ...
                                        logger.debug(f"Failed to remove duplicate file: {rm_err}")
                                    is_pixel_dup = True
                                    break
                            if not is_pixel_dup:
                ...
                logger.debug(f"Download skip: {e}")
                ...
                try:
                    logger.debug("DL-DIAG: Attempting blob fetch rescue (dialog still open)...")
                    img_data = await self._e._page.evaluate('''async (imgEl) => {
                        try {
                            const dialogImg = document.querySelector('img[data-test-id="trusted-image"]') ||
                                              document.querySelector('mat-dialog-container img.generated-image') ||
                                              document.querySelector('mat-dialog-container img') ||
                    ...
                    logger.debug(f"Blob rescue exception: {fallback_err}")
                    try:
                        await self._e._page.keyboard.press("Escape")
                    except:
                        pass
    ...
    async def delete_history(self, range_name: str = "Last hour"):
        ...
        logger.debug(f"Initiating history deletion: {range_name}")
        ...
        try:
            ...
            for _ in range(2):
                ...
                for selector in pre_dismiss_selectors:
                    ...
                    if await btn.is_visible():
                        ...
                        logger.debug(f"Pre-deletion: Dismissing blocker ({btn_text})...")
                        await btn.click()
                        await asyncio.sleep(1.0)
                        dismiss_found = True
            ...
            delete_btn = self._e._page.locator('button[aria-label="Delete"]').first
            if not await delete_btn.is_visible():
                logger.debug("Delete button not visible. Trying to scroll or force dismiss any overlays...")
                await self._e._page.mouse.click(10, 10)  # Click corner to lose focus/dismiss lightboxes
                await self._e._page.keyboard.press("PageDown")
                await asyncio.sleep(1.0)
                if not await delete_btn.is_visible():
                    logger.debug("Delete button still not visible on activity page.")
            ...
            if range_name == "All time":
                logger.debug("Searching for 'Always' or 'All time' option...")
                option = self._e._page.locator('li[role="menuitem"]').filter(has_text=re.compile(r"^(Always|All time)$", re.I)).first
            else:
                option = self._e._page.locator(f'li[role="menuitem"]:has-text("{target_text}")')
            ...
            logger.debug("Checking for post-selection dialogs...")
            for _ in range(4):
                dialog_handled = False

                modal = self._e._page.locator('div.llhEMd, div.VfPpkd-Sx9N0d').first
                if await modal.is_visible():
                    ...
                    if await no_activity_text.is_visible():
                        ...
                        if await close_btn.is_visible():
                            logger.debug("Gemini Activity: No activity found to delete. Closing...")
                    ...
                    if await modal_delete_btn.is_visible():
                        logger.debug("Gemini Activity: Deleting confirmed items...")
            ...
            logger.debug("Monitoring for snackbar feedback...")
            # Locator for snackbar/alert
            snackbar = self._e._page.locator('[role="alert"], [role="status"]').first
            ...
            for _ in range(10):  # 10 seconds timeout
                if await snackbar.is_visible():
                    ...
                    if msg:
                        ...
                        logger.debug(f"Gemini Activity: {flat_msg}")
            ...
            logger.debug(f"Error during history deletion: {e}")
            return {"status": "error", "message": str(e)}
    ...
    async def get_account_info(self):
        ...
        try:
            await self._e._page.wait_for_load_state("networkidle", timeout=3000)
        except Exception:
        ...
        avatar_locators = self._e._page.locator(avatar_selectors)
        ...
        signin_locators = self._e._page.locator(signin_selectors)
    ...
    async def get_gem_title(self) -> dict:
        ...
        try:
            ...
            title_text = await self._e._page.evaluate('''() => {
            ...
            logger.debug(f"Error extracting gem title: {e}")
            return {"status": "error", "message": str(e)}
